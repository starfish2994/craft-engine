...
public class PlayerAttemptsPlaceCEBlockScriptEvent extends BukkitScriptEvent implements Listener {

    // <--[event]
    // @Events
    // player attempts to place ce block <'block'>
    //
    // @Group Player
    //
    // @Location true
    ...
    //
    // @Triggers when a player is about to place a CraftEngine custom block, before the placement happens.
    // Cancelling the event prevents the placement.
    // The word "ce" in the event line can also be written as "craftengine".
    //
    // @Context
    // <context.location> returns the LocationTag the block would be placed at.
    // <context.id> returns the CraftEngine block id of the block being placed, like "default:togglable_light_block".
    ...
    //
    // @Example
    // # Prevent placing any CraftEngine block in a region.
    // on player attempts to place ce block block in:no_build_zone:
    // - narrate "You can't place that here!"
    // - determine cancelled
    // -->

    public PlayerAttemptsPlaceCEBlockScriptEvent() {
        registerCouldMatcher("player attempts to place ce block <'block'>");
        registerCouldMatcher("player attempts to place craftengine block <'block'>");
        registerSwitches("with");
    }

    public LocationTag location;
    public String id;
    public CustomBlockAttemptPlaceEvent event;
    ...
    @EventHandler
    public void onPlayerAttemptsPlaceCEBlock(CustomBlockAttemptPlaceEvent event) {
