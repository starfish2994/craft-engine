...
public final class LegacyNetworkItemHandler implements NetworkItemHandler {
    ...
    public Optional<Item> c2s(Item wrapped) {
        ...
        CompoundTag networkData = NetworkEncryption.decrypt(wrapped.getSparrowTag(NETWORK_ITEM_TAG));
        if (networkData != null) {
            forceReturn = true;
            // 移除tag
    ...
    public Optional<Item> s2c(Item wrapped, @Nullable Player player) {
        ...
        // 如果tag不空，则需要返回
        if (!tag.isEmpty()) {
            wrapped.setTag(NetworkEncryption.encrypt(tag), NETWORK_ITEM_TAG);
            forceReturn = true;
        }
        return forceReturn ? Optional.of(wrapped) : Optional.empty();
    }
    ...
    static class OtherItem {
        ...
        public Optional<Item> process(Context context) {
            ...
            if (this.globalChanged) {
                this.item.setTag(NetworkEncryption.encrypt(this.networkTag), NETWORK_ITEM_TAG);
